...
func show_tutorial_dialogue() -> void:
	...
	current_dialogue = [
		{"name": "", "text": "(After the goblin attack, The two have been wandering around the woods searching.)"},
		{"name": "Buddy", "text": "Where could it be? I swear, Multiplication is in this forest, right?"},
		{"name": "Teacher", "text": "Calm down, we will eventually find it....."},
		{"name": "Teacher", "text": "Watch OUT! They're atacking again!"}
	]
...
func show_end_stage_dialogue() -> void:
	...
	if player_won:
		current_dialogue = [
			{"name": "Teacher", "text": "We've beaten them back again."},
			{"name": "Buddy", "text": "Yeah, but can we rest for now?"},
			{"name": "Teacher", "text": "Why?"},
			{"name": "Teacher", "text": "We've been searching for it for quite a while now and we still didnt find it."},
			{"name": "Teacher", "text": "Not only that, we've been under attack constantly."},
			{"name": "Teacher", "text": "So, we could use some rest to recover and we'll continue tommorrow."},
			{"name": "Teacher", "text": "Agree, let's find somewhere safe and set up camp there to rest."},
			{"name": "", "text": "(The two search for a safe place in the forest until they found one and camped there to rest.)"}
		]
	elif survived_all:
		current_dialogue = [
...
func end_game() -> void:
	...
	if player_won:
		PlayerData.complete_stage(5)  # or 19 or current stage ID
