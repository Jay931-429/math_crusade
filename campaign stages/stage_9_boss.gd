...
var artifacts = {
	"dormant_star": {
		...
		"image": preload("res://asset/UI/5-Photoroom.png"),  # Replace with your image path
...
func show_end_stage_dialogue() -> void:
	...
	if player_won:
		current_dialogue = [
			{"name": "", "text": "(After a grueling fight, the rebel realized that this is the end for her.)"},
			{"name": "", "text": "(So in the final act of defiance, she charged.)"},
			{"name": "RebMil", "text": "I will not give up! Down with the..."},
			{"name": "", "text": "(Lady Hilda responds and dealt the final blow.)"},
...
# Save for UI
	...
	if player_won:
		PlayerData.complete_stage(9)  # or 19 or current stage ID
