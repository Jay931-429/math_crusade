...
func show_tutorial_dialogue() -> void:
	...
	current_dialogue = [
		{"name": "Buddy", "text": "Ugh, this desert is never-ending... Aric, are we even close?"},
		{"name": "Teacher", "text": "Patience, Mathie. The Golden Calculator won't retrieve itself. But, while we walk, let's discuss division."},
		{"name": "Buddy", "text": "Sure."},
		{"name": "Buddy", "text": "Division."},
		{"name": "Buddy", "text": "Division is a way to find out how many times one number fits into another, or how to split something into equal parts."},
		{"name": "Buddy", "text": "For example, if you have 12 cookies and want to share them among 4 people..."},
		{"name": "Teacher", "text": "You'd divide 12 by 4! That's 3 cookies per person!"},
		{"name": "Buddy", "text": "Exactly. We write it as 12 ÷ 4 = 3, or 12 / 4 = 3. It's the inverse of multiplication."},
...
func generate_new_problem() -> void:
	...
	# Generate two random numbers, ensuring num2 divides num1
	var num2 = randi() % 9 + 1 # num2 will be between 1 and 9
	var num1 = num2 * (randi() % 9 + 1) # num1 will be a multiple of num2, between num2 and num2*10
...
func show_end_stage_dialogue() -> void:
	...
	if player_won:
		current_dialogue = [
			...
			{"name": "Teacher", "text": "We can't proceed without those they have stolen. We'll have to track them down."},
			{"name": "Buddy", "text": "Right! They won't get away with this. Let's follow their tracks!"}
		]
	elif survived_all:
		current_dialogue = [
...
# Save for UI
	...
	if player_won:
		PlayerData.complete_stage(10)  # or 19 or current stage ID
