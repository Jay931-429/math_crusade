...
# Show initial tutorial dialogue
	...
	# generate_new_problem()
	AudioManager.change_music("stage5")
...
func show_tutorial_dialogue() -> void:
	...
	current_dialogue = [
	{"name": "Teacher", "text": "Finally, we've recovered everything. Those thieves won't trouble us again."},
	{"name": "Buddy", "text": "Yeah! Addition, subtraction, multiplication, division... and all my stuff! We're back in business!"},
	{"name": "Teacher", "text": "Indeed. And now, we stand before these ancient ruins. I sense a dungeon within."},
	{"name": "Teacher", "text": "According to the map, this should lead us to The Architect's Lair."},
	{"name": "Buddy", "text": "Ooh, exciting! But... are we prepared?"},
	{"name": "Teacher", "text": "We must be. Before we venture deeper, let's master another mathematical concept: exponents."},
	{"name": "Buddy", "text": "Exponents? Sounds powerful! So let's begin."},
	{"name": "Buddy", "text": "Exponents"},
	{"name": "Buddy", "text": "An exponent indicates how many times a number is multiplied by itself."},
	{"name": "Buddy", "text": "For example, 2 raised to the power of 3, or 2³, is 2 * 2 * 2."},
	{"name": "Buddy", "text": "So, 2³ equals 8!"},
	{"name": "Buddy", "text": "So in a way, it's a way to express repeated multiplication concisely."},
	{"name": "Teacher", "text": "Got it! So, like, 5² is 5 * 5, which is 25!"},
	{"name": "Buddy", "text": "Exactly. This will be invaluable against the challenges ahead."},
	{"name": "Teacher", "text": "Alright! Were ready for anything! Let's go into the dungeon!"},
	{"name": "", "text": "They enter the ruins, descending into the depths of the dungeon."},
	{"name": "", "text": "Suddenly, skeletal warriors rise from the shadows!"},
	{"name": "Teacher", "text": "Skeletons Incoming!"},
	{"name": "Buddy", "text": "Alright, let's show these skeletons the might of exponents!"}
]
...
func generate_new_problem() -> void:
	...
	# Generate two random numbers for base and exponent
	var base = randi() % 3 + 2  # Base between 2 and 6 (to keep results manageable)
	var exponent = randi() % 2 + 1 # Exponent between 2 and 4 (to keep results manageable)

	# Calculate the exponent result
	current_answer = pow(base, exponent) #pow is the power function in godot

	# Display the exponent problem
...
func show_end_stage_dialogue() -> void:
	...
	if player_won:
		current_dialogue = [
			{"name": "Buddy", "text": "Whoa! We did it!"},
			{"name": "Buddy", "text": "Those skeletons didn't stand a chance against our math powers!"},
			{"name": "Teacher", "text": "Indeed. Your mastery of exponents proved invaluable."},
			{"name": "Teacher", "text": "I'm starting to really get the hang of this."},
			{"name": "Buddy", "text": "I feel like we can take on anything now!"},
			{"name": "Teacher", "text": "Don't get too cocky, Mathie. We've only just scratched the surface of this dungeon."},
			{"name": "Buddy", "text": "Right, right. Still, that was awesome!"},
			{"name": "Teacher", "text": "Let us proceed cautiously. There may be more challenges ahead."},
			{"name": "Teacher", "text": "We should also be mindful of our resources. We don't know how long this dungeon is."},
			{"name": "Teacher", "text": "Make sure to remember what we've learned, all the operations will be useful."},
			{"name": "Buddy", "text": "Got it! I'm ready when you are, Sir Aric."},
			{"name": "Teacher", "text": "Then let us continue. The Golden Calculator awaits."},
			{"name": "Buddy", "text": "To the depths!"}
	]
	elif survived_all:
		current_dialogue = [
...
# Save for UI
	...
	if player_won:
		PlayerData.complete_stage(13)  # or 19 or current stage ID
